...
2. **Konsolidieren und adaptieren des neuen Datenmodells:** Passe jedes vorhandene Feature an dieses Datenmodell an. Füge hier für jedes Feature ein Erledigt ein. Nach diesem ToDo entstehen somit neue ToDos. Die Software muss an dieser Stelle konsolidiert werden. Dieses ToDo hat somit mehrere Iterationen.
...
4. **Module auf den Store umstellen:** Ersetze die in HTML eingebetteten JSON‑Arrays (z. B. die Fallliste im Dashboard, die Zeit­einträge im Rechnungs‑Wizard und die Rechnungen in der Offene‑Posten‑Übersicht) durch Abfragen an den zentralen Store.  Das Mandats‑Wizard schreibt neue Fälle mit `addCase()`, die Zeiterfassung speichert Einträge mit `addTimeEntry()` und alle Übersichten lesen die Daten dynamisch.  Registriere Event‑Listener, damit UI‑Komponenten automatisch aktualisiert werden, sobald sich Daten ändern.  Entferne Duplikate und redundante Mock‑Daten aus den HTML‑Dateien.
